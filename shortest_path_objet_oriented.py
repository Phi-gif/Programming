...
Metro = pd.read_excel("/Users/verron/Desktop/fin/Data_treated.xlsx")
...
class CoordGPS:
    ...
    def temps_marche(self):
        ...
        liste_temps_marche=[int(i/1.1) for i in list(distance['Distance'])]
        return(liste_temps_marche)

        
            
class Réseau(CoordGPS):
    
    def __init__(self):
        self.liste_station=list(Metro['Station'].drop_duplicates(keep='first'))
        self.Geopoints=[]
        for coordonnées in list(Metro['Geopoint'].drop_duplicates(keep='first')):
    ...
    def adjency_matrix():
        ...
        Graphe_réseau.add_nodes_from(Metro['Station'].drop_duplicates(keep='first').reset_index(drop=True))
        Graphe_réseau.add_edges_from(Réseau.Tuple())
        matrice_adjacence=nx.to_numpy_matrix(Graphe_réseau,nodelist=list(Metro['Station'].drop_duplicates(keep='first')))
    ...
    def ConstructPath(i,j):
        mat_pred=Réseau.Floyd_Warshall()[1]
    ...
    def Shortest_path(départ,destination):
        ...
        for i in closest_stat_pt_dep:
            for j in closest_stat_pt_arr:
                liste_chemins.append(Réseau.ConstructPath(i,j))
        for liste in liste_chemins:
            time=0
            if len(tps_marche_dep)==1 and len(tps_marche_arr)==1:
                time=143*(len(liste)-1)+tps_marche_dep[0]+tps_marche_arr[0]
            elif len(tps_marche_dep)==1 and len(tps_marche_arr)>1:
                time=143*(len(liste)-1)+tps_marche_dep[0]+tps_marche_arr[k]
            elif len(tps_marche_dep)>1 and len(tps_marche_arr)==1:
                time=143*(len(liste)-1)+tps_marche_dep[k]+tps_marche_arr[0]
            else :
                time=143*(len(liste)-1)+tps_marche_dep[k]+tps_marche_arr[k]
            for i in range(1,len(liste)-1):
                if liste[i] in list(Corres['index']) and Solo['Ligne'][liste[i-1]]==Solo['Ligne'][liste[i+1]]:
                    time+=240
            k+=1
            Temps.append(time)
        less_time = min(Temps)
    ...
    def Trace(self,départ,destination):
        num_stat=Réseau.Shortest_path(départ,destination)[0]
    ...
    def ordre_station(self,départ,destination):
        chemin,temps=Réseau.Shortest_path(départ,destination)
        chemin_ord=[station for station in reversed(chemin)]
        ligne=[]
        for i in range(len(chemin_ord)-1):
            if Solo['Ligne'][chemin_ord[i]]==Solo['Ligne'][chemin_ord[i+1]]:
                ligne.append(Solo['Ligne'][chemin_ord[i]])
                
            if Solo['Ligne'][chemin_ord[i]]==Solo['Corresp. 1'][chemin_ord[i+1]]:
                ligne.append(Solo['Ligne'][chemin_ord[i]])
                
            if Solo['Corresp. 1'][chemin_ord[i]]==Solo['Ligne'][chemin_ord[i+1]]:
                ligne.append(Solo['Corresp. 1'][chemin_ord[i]])
                
            if Solo['Corresp. 2'][chemin_ord[i]]==Solo['Ligne'][chemin_ord[i+1]]:
                ligne.append(Solo['Corresp. 2'][chemin_ord[i]]) 
                
            if Solo['Corresp. 3'][chemin_ord[i]]==Solo['Ligne'][chemin_ord[i+1]]:
                ligne.append(Solo['Corresp. 3'][chemin_ord[i]])
                
            if Solo['Ligne'][chemin_ord[i]]==Solo['Corresp. 2'][chemin_ord[i+1]]:
                ligne.append(Solo['Ligne'][chemin_ord[i]])
                
            if Solo['Ligne'][chemin_ord[i]]==Solo['Corresp. 3'][chemin_ord[i+1]]:
                ligne.append(Solo['Ligne'][chemin_ord[i]])  
...
if __name__=="__main__":
    point_de_départ=CoordGPS(19.268,-99.231)
    point_arrivée=CoordGPS(19.555,-98.942)
#    point_de_départ=CoordGPS(19.34400644,-99.14105415)  #Tasquena
#    point_arrivée=CoordGPS(19.49054404,-99.14638638)    #instituto del petroleo
    Mexico_subway=Réseau()
    Mexico_subway.ordre_station(point_de_départ,point_arrivée)
    Mexico_subway.Trace(point_de_départ,point_arrivée)

#19.37360387,-99.17859435
